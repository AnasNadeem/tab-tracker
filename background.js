...
function cleanupOldEntries() {
	...
	chrome.storage.local.get().then((result) => {
		...
		for (const tabId in result) {
			...
			// Validate this is a tab object created by our extension
			if (!tab || typeof tab !== 'object' || !tab.hasOwnProperty('startTime')) {
				continue;
			}
		...
		if (Object.keys(updatesToSet).length > 0) {
			promises.push(chrome.storage.local.set(updatesToSet));
		}

		if (toRemove.length > 0) {
			promises.push(chrome.storage.local.remove(toRemove).then(() => {
